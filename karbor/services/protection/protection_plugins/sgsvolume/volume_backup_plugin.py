...
from karbor.services.protection.protection_plugins.sgsvolume \
...
class RestoreOperation(protection_plugin.Operation):
    ...
    def on_main(self, checkpoint, resource, context, parameters, **kwargs):
        ...
        backup_id = resource_metadata['backup_id']
        backup_destination = resource_metadata['backup_destination']
        ...
        if is_success:
            ...
            restore_reference.put_resource(resource_id, self.volume_id)
            if import_backup:
                try:
                    ...
                    LOG.info('Backup id: %s not found. Assuming deleted',
                             backup_id)
                utils.status_poll(
                    partial(get_backup_status, sgs_client, import_backup.id),
                    interval=self._interval,
...
class DeleteOperation(protection_plugin.Operation):
    ...
    def on_main(self, checkpoint, resource, context, parameters, **kwargs):
        ...
        try:
            ...
            try:
                backup = sgs_client.backups.get(backup_id)
                sgs_client.backups.delete(backup)
            except sgs_exc.NotFound:
                LOG.info('Backup id: %s not found. Assuming deleted',
                         backup_id)
            is_success = utils.status_poll(
