...
class ProtectOperation(protection_plugin.Operation):
    ...
    def on_main(self, checkpoint, resource, context, parameters, **kwargs):
        ...
        if replicate_volume is None:
            replicate_name = parameters.get('replicate_name',
                                            'replicate-%s' % volume_id)
        ...
        replication_name = parameters.get("replication_name",
                                          "karbor-%s" % volume_id)
...
class RestoreOperation(protection_plugin.Operation):
    ...
    def on_main(self, checkpoint, resource, context, parameters, **kwargs):
        ...
        try:
            ...
            sgs_client.replications.enable(replication_id)
            is_success = utils.status_poll(
                partial(get_replication_status, sgs_client, replication_id),
                interval=self._interval,
                success_statuses={'enabled'},
            ...
            if not is_success:
                reason = 'Enable replication failed. ' \
                         'Replication is in erroneous state'
                update_method(constants.RESOURCE_STATUS_ERROR, reason)
                raise exception.RestoreResourceFailed(
                    reason=reason,
        ...
        if attached_instance:
            ...
            instance = restore_reference.get_reference(
                attached_instance)
            sgs_client.volumes.attach(replicate_volume, instance)
            is_success = self._check_attach_complete(sgs_client,
                                                     replicate_volume)
            if not is_success:
                reason = 'Attach volume:%s to instance:%s' % (
                    replicate_volume, instance)
                update_method(constants.RESOURCE_STATUS_ERROR, reason)
    ...
    def on_main(self, checkpoint, resource, context, parameters, **kwargs):
        ...
        if resource_metadata is not None:
            try:
                ...
                is_success = utils.status_poll(
                    partial(get_cinder_volume_status, cinder_client,
                            replicate_volume),
                    interval=self._interval,
                    success_statuses={'deleted', 'not-found'},
                    failure_statuses={'error'},
