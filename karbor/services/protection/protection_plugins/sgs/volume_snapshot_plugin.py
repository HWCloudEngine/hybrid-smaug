...
class ProtectOperation(protection_plugin.Operation):
    ...
    def on_main(self, checkpoint, resource, context, parameters, **kwargs):
        ...
        if snapshot_name is None:
            snapshot_name = "karbor-snapshot-sgs-volume-%s" % volume_id
...
class SGSSnapshotProtectionPlugin(protection_plugin.ProtectionPlugin):
    _SUPPORT_RESOURCE_TYPES = [constants.SGVOLUME_RESOURCE_TYPE]

    def __init__(self, config=None):
        super(SGSSnapshotProtectionPlugin, self).__init__(config)
        self._config.register_opts(sgs_snapshot_opts,
                                   'sgs_snapshot_protection_plugin')
        self._plugin_config = self._config.sgs_snapshot_protection_plugin
